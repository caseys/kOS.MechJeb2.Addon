...
export function createServer(): McpServer {
  ...
  server.tool(
    ...
    'Execute the next maneuver node using MechJeb autopilot. Validates delta-v availability and sets up auto-staging if needed. Will warp to node and perform the burn.',
    {},
    async () => {
      try {
        const conn = getConnection();
        ...
        if (stagingRequired) {
          await conn.execute(
            'WHEN STAGE:DELTAV:CURRENT < 1 THEN { STAGE. }'
          );
        }
