...
export class AscentHandle {
  ...
  async getProgress(): Promise<AscentProgress> {
    ...
    const enabledResult = await this.conn.execute('PRINT ADDONS:MJ:CORE:ASCENT:ENABLED.');
  ...
  async waitForCompletion(pollIntervalMs = 5000): Promise<AscentResult> {
    ...
    while (Date.now() - startTime < MAX_WAIT_MS) {
      ...
      const statusResult = await this.conn.execute(
        'PRINT "ENABLED:" + ADDONS:MJ:CORE:ASCENT:ENABLED. ' +
      ...
      if (!enabled || inOrbit) {
        ...
        if (inOrbit && enabled) {
          await this.conn.execute('SET ADDONS:MJ:CORE:ASCENT:ENABLED TO FALSE.');
        }
...
export class AscentProgram {
  ...
  async waitForMechJebReady(): Promise<void> {
    ...
    for (let i = 0; i < MAX_ATTEMPTS; i++) {
      const result = await this.conn.execute('PRINT ADDONS:MJ:CORE:ASCENT:ENABLED.');
  ...
  async configure(settings: Partial<AscentSettings>): Promise<void> {
    ...
    const AG = 'ADDONS:MJ:CORE:ASCENT';
  ...
  async setEnabled(enabled: boolean): Promise<void> {
    await this.conn.execute(`SET ADDONS:MJ:CORE:ASCENT:ENABLED TO ${enabled ? 'TRUE' : 'FALSE'}.`);
  }
  ...
  async launchToOrbit(options: LaunchOptions): Promise<AscentHandle> {
    ...
    const launchResult = await this.conn.execute(
      'SET ADDONS:MJ:CORE:ASCENT:ENABLED TO TRUE. ' +
      'UNLOCK THROTTLE. SAS OFF. STAGE. ' +
      'PRINT ADDONS:MJ:CORE:ASCENT:ENABLED.'
    );
    if (!launchResult.output.includes('True')) {
      throw new Error('[Ascent] Autopilot failed to engage!');
    }
