...
export async function getVesselState(conn: KosConnection): Promise<VesselState> {
  ...
  const [altTrue, altASL, speedSurf, speedOrb, speedVert] = await queryNumbers(conn, [
    'ADDONS:MJ:CORE:VESSEL:ALTITUDETRUE',
    'ADDONS:MJ:CORE:VESSEL:ALTITUDEASL',
    'ADDONS:MJ:CORE:VESSEL:SPEEDSURFACE',
    'ADDONS:MJ:CORE:VESSEL:SPEEDORBITAL',
    'ADDONS:MJ:CORE:VESSEL:SPEEDVERTICAL'
  ]);

  const [heading, pitch, roll] = await queryNumbers(conn, [
    'ADDONS:MJ:CORE:VESSEL:VESSELHEADING',
    'ADDONS:MJ:CORE:VESSEL:VESSELPITCH',
    'ADDONS:MJ:CORE:VESSEL:VESSELROLL'
  ]);

  const [dynPressure, aoa, mach] = await queryNumbers(conn, [
    'ADDONS:MJ:CORE:VESSEL:DYNAMICPRESSURE',
    'ADDONS:MJ:CORE:VESSEL:AOA',
    'ADDONS:MJ:CORE:VESSEL:MACH'
  ]);

  const [lat, lon] = await queryNumbers(conn, [
    'ADDONS:MJ:CORE:VESSEL:LATITUDE',
    'ADDONS:MJ:CORE:VESSEL:LONGITUDE'
  ]);
...
export async function getMechJebInfo(conn: KosConnection): Promise<MechJebInfo> {
  ...
  const surfTwr = await safeQueryNumber(conn, 'ADDONS:MJ:CORE:INFO:SURFACETWR');
  const localTwr = await safeQueryNumber(conn, 'ADDONS:MJ:CORE:INFO:LOCALTWR');
  const thrust = await safeQueryNumber(conn, 'ADDONS:MJ:CORE:INFO:CURRENTTHRUST');
  const maxThrust = await safeQueryNumber(conn, 'ADDONS:MJ:CORE:INFO:MAXTHRUST');
  const accel = await safeQueryNumber(conn, 'ADDONS:MJ:CORE:INFO:ACCELERATION');

  // Optional values
  const nextNodeDeltaV = await safeQueryNumber(conn, 'ADDONS:MJ:CORE:INFO:NEXTMANEUVERNODEDELTAV') || undefined;
  const timeToNode = await safeQueryNumber(conn, 'ADDONS:MJ:CORE:INFO:TIMETOMANEUVERNODE') || undefined;
  const timeToImpact = await safeQueryNumber(conn, 'ADDONS:MJ:CORE:INFO:TIMETOIMPACT') || undefined;
  const escapeVel = await safeQueryNumber(conn, 'ADDONS:MJ:CORE:INFO:ESCAPEVELOCITY') || undefined;
