...
async function main() {
  ...
  try {
    ...
    try {
      const agResult = await conn.execute('PRINT ADDONS:MJ:CORE:ASCENTGUIDANCE:SUFFIXNAMES.', 3000);
    ...
    console.log('\nTrying ASCENT path:');
    try {
      const aResult = await conn.execute('PRINT ADDONS:MJ:CORE:ASCENT:SUFFIXNAMES.', 3000);
    ...
    console.log('\nTrying INFO path:');
    try {
      const infoResult = await conn.execute('PRINT ADDONS:MJ:CORE:INFO:SUFFIXNAMES.', 3000);
