 ...
 */
function parseSuffixList(output) {
    const items = [];
 ...
 */
export async function isMechJebAvailable(conn) {
 ...
 * Get MechJeb version
 */
export async function getMechJebVersion(conn) {
    try {
 ...
 */
export async function discoverModules(conn) {
    ...
    // Query core suffixes to see what modules exist
    const result = await conn.execute('PRINT ADDONS:MJ:CORE:SUFFIXNAMES.');
 ...
 */
export async function discoverAscentSuffixes(conn) {
    const result = await conn.execute('PRINT ADDONS:MJ:CORE:ASCENT:SUFFIXNAMES.');
    return parseSuffixList(result.output);
}
/**
 * Discover vessel state suffixes
 */
export async function discoverVesselSuffixes(conn) {
    const result = await conn.execute('PRINT ADDONS:MJ:CORE:VESSEL:SUFFIXNAMES.');
    return parseSuffixList(result.output);
}
/**
 * Discover info suffixes
 */
export async function discoverInfoSuffixes(conn) {
    const result = await conn.execute('PRINT ADDONS:MJ:CORE:INFO:SUFFIXNAMES.');
    return parseSuffixList(result.output);
}
