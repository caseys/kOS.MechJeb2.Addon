 ...
 */
import { queryNumber, queryTime, queryNodeInfo } from './shared.js';
/**
 * Maneuver Program - controls MechJeb maneuver planner
 *
 * Uses kOS.MechJeb2.Addon's MANEUVERPLANNER suffixes:
 *   ADDONS:MJ:CORE:MANEUVERPLANNER:CHANGEPE(altitude, timeRef)
 *   ADDONS:MJ:CORE:MANEUVERPLANNER:CHANGEAP(altitude, timeRef)
 *   ADDONS:MJ:CORE:MANEUVERPLANNER:CIRCULARIZE(timeRef)
 */
export class ManeuverProgram {
     ...
     */
    async adjustPeriapsis(altitude, timeRef = 'APOAPSIS') {
        // Create the maneuver node (returns True/False)
        const cmd = `SET PLANNER TO ADDONS:MJ:CORE:MANEUVERPLANNER. PRINT PLANNER:CHANGEPE(${altitude}, "${timeRef}").`;
     ...
     * @param timeRef When to execute: 'APOAPSIS', 'PERIAPSIS', 'X_FROM_NOW', 'ALTITUDE'
     */
    async adjustApoapsis(altitude, timeRef = 'PERIAPSIS') {
        const cmd = `SET PLANNER TO ADDONS:MJ:CORE:MANEUVERPLANNER. PRINT PLANNER:CHANGEAP(${altitude}, "${timeRef}").`;
        const result = await this.conn.execute(cmd, 10000);
        const success = result.output.includes('True');
        if (!success) {
            return { success: false, error: result.output };
     ...
     */
    async circularize(timeRef = 'APOAPSIS') {
        const cmd = `SET PLANNER TO ADDONS:MJ:CORE:MANEUVERPLANNER. PRINT PLANNER:CIRCULARIZE("${timeRef}").`;
        const result = await this.conn.execute(cmd, 10000);
        const success = result.output.includes('True');
        if (!success) {
            return { success: false, error: result.output };
     ...
     */
    async setTarget(name, type = 'body') {
        const cmd = type === 'body'
     ...
     */
    async hohmannTransfer(timeRef = 'COMPUTED', capture = true) {
        // Check target exists
        if (!await this.hasTarget()) {
            return { success: false, error: 'No target set. Use setTarget() first.' };
        }
        const captureStr = capture ? 'TRUE' : 'FALSE';
        const cmd = `SET PLANNER TO ADDONS:MJ:CORE:MANEUVERPLANNER. PRINT PLANNER:HOHMANN("${timeRef}", ${captureStr}).`;
        const result = await this.conn.execute(cmd, 10000);
        const success = result.output.includes('True');
        if (!success) {
            return { success: false, error: result.output };
     ...
     */
    async courseCorrection(finalPeA) {
        const cmd = `SET PLANNER TO ADDONS:MJ:CORE:MANEUVERPLANNER. PRINT PLANNER:COURSECORRECTION(${finalPeA}).`;
        const result = await this.conn.execute(cmd, 10000);
        const success = result.output.includes('True');
        if (!success) {
            return { success: false, error: result.output };
     ...
     */
    async changeInclination(newInclination, timeRef = 'EQ_NEAREST_AD') {
        const cmd = `SET PLANNER TO ADDONS:MJ:CORE:MANEUVERPLANNER. PRINT PLANNER:CHANGEINCLINATION(${newInclination}, "${timeRef}").`;
        const result = await this.conn.execute(cmd, 10000);
        const success = result.output.includes('True');
        if (!success) {
            return { success: false, error: result.output };
     ...
     * @param timeRef When to execute: 'APOAPSIS', 'PERIAPSIS', 'X_FROM_NOW', 'ALTITUDE'
     */
    async ellipticize(peA, apA, timeRef = 'APOAPSIS') {
        const cmd = `SET PLANNER TO ADDONS:MJ:CORE:MANEUVERPLANNER. PRINT PLANNER:ELLIPTICIZE(${peA}, ${apA}, "${timeRef}").`;
        const result = await this.conn.execute(cmd, 10000);
        const success = result.output.includes('True');
        if (!success) {
            return { success: false, error: result.output };
     ...
     */
    async changeLAN(newLAN, timeRef = 'APOAPSIS') {
        const cmd = `SET PLANNER TO ADDONS:MJ:CORE:MANEUVERPLANNER. PRINT PLANNER:LAN(${newLAN}, "${timeRef}").`;
        const result = await this.conn.execute(cmd, 10000);
        const success = result.output.includes('True');
        if (!success) {
            return { success: false, error: result.output };
     ...
     * @param timeRef When to execute: 'APOAPSIS', 'PERIAPSIS', 'X_FROM_NOW'
     */
    async changeLongitude(newLong, timeRef = 'APOAPSIS') {
        const cmd = `SET PLANNER TO ADDONS:MJ:CORE:MANEUVERPLANNER. PRINT PLANNER:LONGITUDE(${newLong}, "${timeRef}").`;
        const result = await this.conn.execute(cmd, 10000);
        const success = result.output.includes('True');
        if (!success) {
            return { success: false, error: result.output };
     ...
     * @param timeRef When to execute: 'APOAPSIS', 'PERIAPSIS', 'X_FROM_NOW'
     */
    async resonantOrbit(numerator, denominator, timeRef = 'APOAPSIS') {
        const cmd = `SET PLANNER TO ADDONS:MJ:CORE:MANEUVERPLANNER. PRINT PLANNER:RESONANTORBIT(${numerator}, ${denominator}, "${timeRef}").`;
        const result = await this.conn.execute(cmd, 10000);
        const success = result.output.includes('True');
        if (!success) {
            return { success: false, error: result.output };
     ...
     */
    async killRelVel(timeRef = 'CLOSEST_APPROACH') {
        // Check target exists
        if (!await this.hasTarget()) {
            return { success: false, error: 'No target set. Use setTarget() first.' };
        }
        const cmd = `SET PLANNER TO ADDONS:MJ:CORE:MANEUVERPLANNER. PRINT PLANNER:KILLRELVEL("${timeRef}").`;
        const result = await this.conn.execute(cmd, 10000);
        const success = result.output.includes('True');
        if (!success) {
            return { success: false, error: result.output };
