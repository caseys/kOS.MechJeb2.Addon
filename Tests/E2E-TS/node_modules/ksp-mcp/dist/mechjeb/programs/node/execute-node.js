 ...
 */
export async function executeNode(conn, timeoutMs = 240000, pollIntervalMs = 2000) {
    ...
    await conn.execute('SET ADDONS:MJ:CORE:NODEEXECUTOR:ENABLED TO TRUE.', 5000);
    ...
    await conn.execute('SET ADDONS:MJ:CORE:NODEEXECUTOR:ENABLED TO FALSE.', 2000);
    return {
        success: false,
 ...
 */
export async function getNodeProgress(conn) {
    const countResult = await conn.execute('PRINT ALLNODES:LENGTH.', 2000);
    ...
    const statusResult = await conn.execute('PRINT "ETA:" + ROUND(NEXTNODE:ETA) + " THR:" + ROUND(THROTTLE * 100).', 2000);
 ...
 */
export async function isNodeExecutorEnabled(conn) {
    const result = await conn.execute('PRINT ADDONS:MJ:CORE:NODEEXECUTOR:ENABLED.', 2000);
 ...
 * @param conn kOS connection
 */
export async function disableNodeExecutor(conn) {
    await conn.execute('SET ADDONS:MJ:CORE:NODEEXECUTOR:ENABLED TO FALSE.', 2000);
}
