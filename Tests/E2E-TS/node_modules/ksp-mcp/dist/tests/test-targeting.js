...
async function main() {
    ...
    const targetType = process.argv[3] || 'auto';
    ...
    try {
        ...
        const locationResult = await conn.execute('PRINT "Vessel: " + SHIP:NAME. ' +
        ...
        // Get target info
        const verifyResult = await conn.execute('PRINT "Target: " + TARGET:NAME + " (" + TARGET:TYPENAME + ")". ' +
        ...
        const detailsResult = await conn.execute('IF HASTARGET { ' +
            '  PRINT "Distance: " + ROUND(TARGET:DISTANCE / 1000, 1) + " km". ' +
            '  IF TARGET:TYPENAME = "Body" { ' +
            '    PRINT "Radius: " + ROUND(TARGET:RADIUS / 1000, 1) + " km". ' +
            '    IF SHIP:BODY <> TARGET { ' +
        ...
        const getResult = await conn.execute('IF HASTARGET { ' +
        ...
        const clearResult = await conn.execute('IF HASTARGET { ' +
