...
async function main() {
    ...
    try {
        ...
        console.log('\n--- MechJeb Vessel Info ---');
        result = await conn.execute('PRINT ADDONS:MJ:CORE:VESSEL:SPEEDSURFACE.');
        console.log('Surface Speed:', result.output);
        result = await conn.execute('PRINT ADDONS:MJ:CORE:VESSEL:ALTITUDETRUE.');
        console.log('Altitude:', result.output);
        await delay(300);
        // Check ascent guidance
        console.log('\n--- Checking Ascent Module ---');
        result = await conn.execute('PRINT ADDONS:MJ:CORE:ASCENT.');
        console.log('ASCENT:', result.output);
        // Try to get ascent settings
        result = await conn.execute('PRINT ADDONS:MJ:CORE:ASCENT:DESIREDORBITALTITUDE.');
