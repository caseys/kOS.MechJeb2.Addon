...
import { z } from 'zod';
import { handleConnect, handleDisconnect, handleStatus, handleExecute, getConnection, } from './tools/connection-tools.js';
import { listCpusInputSchema, handleListCpus, } from './tools/list-cpus.js';
import { CONNECTION_GUIDE, CPU_MENU_FORMAT, TRANSPORT_OPTIONS, } from './resources/content.js';
...
export function createServer() {
    ...
    server.tool('kos_connect', 'Connect to kOS terminal server and attach to a CPU by ID or label. Returns vessel name and CPU info.', {
    ...
    server.tool('kos_disconnect', 'Disconnect from kOS terminal', {}, async () => {
        try {
    ...
    server.tool('kos_status', 'Get current kOS connection status', {}, async () => {
    ...
    server.tool('kos_list_cpus', 'List available kOS CPUs without connecting. Useful for discovering CPU tags/labels.', listCpusInputSchema.shape, async (args) => {
        try {
    ...
    server.tool('kos_execute', 'Execute a raw kOS command and return the output. Use for advanced operations or debugging.', {
    ...
    server.tool('mechjeb_adjust_periapsis', 'Create a maneuver node to change periapsis using MechJeb. ' +
    ...
    server.tool('mechjeb_adjust_apoapsis', 'Create a maneuver node to change apoapsis using MechJeb.', {
    ...
    server.tool('mechjeb_circularize', 'Create a maneuver node to circularize the orbit using MechJeb.', {
        timeRef: z.enum(['APOAPSIS', 'PERIAPSIS', 'X_FROM_NOW', 'ALTITUDE'])
            .default('APOAPSIS')
    ...
    server.tool('mechjeb_execute_node', 'Execute the next maneuver node using MechJeb autopilot. Validates delta-v availability and sets up auto-staging if needed. Will warp to node and perform the burn.', {}, async () => {
        try {
            const conn = getConnection();
            if (!conn.isConnected()) {
                return {
            ...
            // 1. Check if there's a node to execute
            const hasNodeResult = await conn.execute('PRINT HASNODE.');
            if (!hasNodeResult.output.includes('True')) {
                return {
                    content: [{ type: 'text', text: 'No maneuver node to execute. Create one first.' }],
                    isError: true,
                };
            }
            // 2. Query required delta-v
            const dvRequiredResult = await conn.execute('PRINT NEXTNODE:DELTAV:MAG.');
            // Extract last numeric value from output (actual kOS result)
            const dvRequiredMatch = dvRequiredResult.output.match(/[\d.]+(?!.*[\d.])/);
            const dvRequired = dvRequiredMatch ? parseFloat(dvRequiredMatch[0]) : 0;
            // 3. Query current stage delta-v and stage number
            const dvCurrentResult = await conn.execute('PRINT STAGE:DELTAV:CURRENT.');
            const dvCurrentMatch = dvCurrentResult.output.match(/[\d.]+(?!.*[\d.])/);
            const dvCurrent = dvCurrentMatch ? parseFloat(dvCurrentMatch[0]) : 0;
            const stageNumResult = await conn.execute('PRINT STAGE:NUMBER.');
            const stageNum = parseInt(stageNumResult.output.match(/\d+(?!.*\d)/)?.[0] || '0');
            // 5. Validation logic
            let stagingRequired = false;
            if (dvCurrent >= dvRequired) {
                // Current stage is sufficient
                stagingRequired = false;
            }
            else if (stageNum > 0) {
                // Check next stage
                const dvNextResult = await conn.execute(`PRINT SHIP:STAGEDELTAV(${stageNum - 1}):CURRENT.`);
                const dvNext = parseFloat(dvNextResult.output.match(/[\d.]+(?!.*[\d.])/)?.[0] || '0');
                const dvTotal = dvCurrent + dvNext;
                if (dvTotal >= dvRequired) {
                    // Current + next stage is sufficient
                    stagingRequired = true;
                }
                else {
                    // Insufficient even with next stage
                    return {
                        content: [{
                                type: 'text',
                                text: `❌ Insufficient delta-v for burn!\n\n` +
                                    `Required: ${dvRequired.toFixed(1)} m/s\n` +
                                    `Available in current stage: ${dvCurrent.toFixed(1)} m/s\n` +
                                    `Available in next stage: ${dvNext.toFixed(1)} m/s\n` +
                                    `Total available (2 stages): ${dvTotal.toFixed(1)} m/s\n` +
                                    `Deficit: ${(dvRequired - dvTotal).toFixed(1)} m/s\n\n` +
                                    `Cannot execute this burn. Consider:\n` +
                                    `- Reducing burn magnitude\n` +
                                    `- Splitting into multiple smaller burns\n` +
                                    `- Adding more fuel/engines`
                            }],
                        isError: true,
                    };
                }
                ...
                // Last stage (stage 0) with insufficient delta-v
                return {
                    content: [{
                            type: 'text',
                            text: `❌ Insufficient delta-v for burn!\n\n` +
                                `Required: ${dvRequired.toFixed(1)} m/s\n` +
                                `Available: ${dvCurrent.toFixed(1)} m/s (final stage)\n` +
                                `Deficit: ${(dvRequired - dvCurrent).toFixed(1)} m/s\n\n` +
                                `This is your final stage. Cannot execute this burn.`
                        }],
                    isError: true,
                };
            }
            // 6. Set up auto-staging if required (fires once, no PRESERVE)
            if (stagingRequired) {
                await conn.execute('WHEN STAGE:DELTAV:CURRENT < 1 THEN { STAGE. }');
            }
            // 7. Execute burn with retry logic
            const maxRetries = 3;
            const checkInterval = 5000; // 5 seconds
            const maxBurnTime = 600000; // 10 minutes
            let burnComplete = false;
            for (let attempt = 1; attempt <= maxRetries; attempt++) {
                if (attempt > 1) {
                    // Abort previous attempt
                    await conn.execute('SET ADDONS:MJ:CORE:NODE:ENABLED TO FALSE.');
                    await conn.execute('SET THROTTLE TO 0. UNLOCK THROTTLE. UNLOCK STEERING.');
                    await new Promise(resolve => setTimeout(resolve, 2000));
                }
                // Enable the node executor
                await conn.execute('SET ADDONS:MJ:CORE:NODE:ENABLED TO TRUE.');
                // Perform time warp kick to trigger MechJeb's auto warp
                // (MechJeb often gets stuck during alignment and won't start warping)
                await immediateTimeWarpKick(conn);
                // Install post-burn time warp kick trigger (10 second buffer)
                // This helps MechJeb resume warping after the burn for next maneuver
                await installTimeWarpKickTrigger(conn, 'NOT HASNODE', 10);
                // Monitor the burn
                const startTime = Date.now();
                let lastStatus = '';
                while (Date.now() - startTime < maxBurnTime) {
                    await new Promise(resolve => setTimeout(resolve, checkInterval));
                    // Check if node still exists
                    const hasNodeCheck = await conn.execute('PRINT HASNODE.');
                    if (!hasNodeCheck.output.includes('True')) {
                        // Node is gone - burn complete!
                        burnComplete = true;
                        break;
                    }
                    // Check if executor is still enabled
                    const executorCheck = await conn.execute('PRINT ADDONS:MJ:CORE:NODE:ENABLED.');
                    const executorEnabled = executorCheck.output.includes('True');
                    if (!executorEnabled) {
                        // Executor stopped but node still exists - incomplete burn
                        const remainingResult = await conn.execute('PRINT NEXTNODE:DELTAV:MAG.');
                        const remainingMatch = remainingResult.output.match(/[\d.]+(?!.*[\d.])/);
                        const remaining = remainingMatch ? parseFloat(remainingMatch[0]) : 0;
                        if (remaining < 0.5) {
                        ...
                        lastStatus = `Burn stopped prematurely (${remaining.toFixed(1)} m/s remaining)`;
                        break;
                    }
                    // Still burning - continue monitoring
                }
                if (burnComplete) {
                    break;
                }
                // If we get here, burn was incomplete
                if (attempt < maxRetries) {
                    lastStatus += ` - retrying (attempt ${attempt + 1}/${maxRetries})`;
                }
            }
            if (!burnComplete) {
                return {
                    content: [{
                            type: 'text',
                            text: `❌ Burn failed after ${maxRetries} attempts.\n\n` +
                                `The MechJeb node executor kept stopping mid-burn.\n` +
                                `This is a known MechJeb reliability issue.\n\n` +
                                `You may need to execute the burn manually using kOS commands.`
                        }],
                    isError: true,
                };
            }
            // Verify burn completion
            const finalNodeCheck = await conn.execute('PRINT HASNODE.');
            if (finalNodeCheck.output.includes('True')) {
                const remainingResult = await conn.execute('PRINT NEXTNODE:DELTAV:MAG.');
                const remainingMatch = remainingResult.output.match(/[\d.]+(?!.*[\d.])/);
                const remaining = remainingMatch ? parseFloat(remainingMatch[0]) : 0;
                if (remaining > 0.5) {
                    return {
                        content: [{
                                type: 'text',
                                text: `⚠️ Burn incomplete!\n\n` +
                                    `Remaining delta-v: ${remaining.toFixed(1)} m/s\n\n` +
                                    `The burn did not complete fully.`
                            }],
                        isError: true,
                    };
                }
            ...
            const telemetry = await getShipTelemetry(conn);
            // 8. Return success message with telemetry
            return {
                content: [{
                        type: 'text',
                        text: `✅ Burn executed successfully!\n\n` +
                            `Delta-v burned: ${dvRequired.toFixed(1)} m/s\n` +
                            (stagingRequired ? `Auto-staging: Used\n` : ``) +
                            telemetry
                    }],
            };
        }
    ...
    server.tool('mechjeb_set_target', 'Set the navigation target for transfer maneuvers.', {
    ...
    server.tool('mechjeb_hohmann_transfer', 'Plan a Hohmann transfer to the current target. Requires a target to be set first (use mechjeb_set_target).', {
    ...
    server.tool('mechjeb_course_correction', 'Fine-tune closest approach to target. Optimizes periapsis for body targets or closest approach for vessel targets. Requires target to be set first.', {
    ...
    server.tool('mechjeb_change_inclination', 'Change orbital inclination. Most efficient when executed at equatorial crossings (ascending/descending nodes).', {
    ...
    server.tool('mechjeb_ellipticize', 'Create a maneuver node to set both periapsis and apoapsis in a single burn.', {
    ...
    server.tool('mechjeb_change_semimajor', 'Create a maneuver node to change the orbital semi-major axis.', {
    ...
    server.tool('mechjeb_change_eccentricity', 'Create a maneuver node to change orbital eccentricity.', {
    ...
    server.tool('mechjeb_change_lan', 'Create a maneuver node to change the Longitude of Ascending Node (LAN).', {
    ...
    server.tool('mechjeb_change_longitude_of_periapsis', 'Create a maneuver node to change the Longitude of Periapsis.', {
    ...
    server.tool('mechjeb_match_plane', 'Create a maneuver node to match orbital plane with the target. Requires a target to be set first.', {
    ...
    server.tool('mechjeb_kill_relative_velocity', 'Create a maneuver node to match velocity with the target. Requires a target to be set first.', {
    ...
    server.tool('mechjeb_resonant_orbit', 'Create a maneuver node to establish a resonant orbit. Useful for satellite constellation deployment.', {
    ...
    server.tool('mechjeb_return_from_moon', 'Create a maneuver node to return from a moon to its parent body.', {
    ...
    server.tool('mechjeb_interplanetary_transfer', 'Create a maneuver node for an interplanetary transfer. Requires a target planet to be set first.', {
    ...
    // Node Execution Tool
    server.tool('mechjeb_execute_node', 'Execute the next maneuver node using MechJeb autopilot. Enables the node executor and monitors until completion.', {
    ...
    server.tool('mechjeb_launch_to_orbit', 'Launch to orbit using MechJeb ascent guidance. Configures and enables autopilot, triggers first stage. ' +
    ...
    server.tool('mechjeb_ascent_status', 'Get current ascent progress including phase, altitude, apoapsis, and periapsis.', {}, async () => {
        try {
            const conn = getConnection();
            if (!conn.isConnected()) {
                return {
            ...
            if (currentAscentHandle) {
                ...
                const enabledResult = await conn.execute('PRINT ADDONS:MJ:CORE:ASCENTGUIDANCE:ENABLED.');
    ...
    server.tool('mechjeb_abort_ascent', 'Abort the current ascent guidance. Disables MechJeb autopilot.', {}, async () => {
        try {
            const conn = getConnection();
            if (!conn.isConnected()) {
                return {
            ...
            if (currentAscentHandle) {
                ...
                // Disable directly
                await conn.execute('SET ADDONS:MJ:CORE:ASCENTGUIDANCE:ENABLED TO FALSE.');
            }
            return {
                content: [{
                        type: 'text',
    ...
    // Targeting Tools
    server.tool('kos_set_target', 'Set the target for maneuvers (celestial body or vessel). Required for Hohmann transfers, rendezvous, etc.', {
        ...
        try {
            ...
            // Get target info
            const infoResult = await conn.execute('PRINT "Target: " + TARGET:NAME + " (" + TARGET:TYPENAME + ")". ' +
    ...
    server.tool('kos_get_target', 'Get information about the current target.', {}, async () => {
        try {
            const conn = getConnection();
            if (!conn.isConnected()) {
                return {
            ...
            const result = await conn.execute('IF HASTARGET { ' +
    ...
    server.tool('kos_clear_target', 'Clear the current target.', {}, async () => {
        try {
            const conn = getConnection();
            if (!conn.isConnected()) {
                return {
    ...
    server.resource('kos-connection-guide', 'kos://connection-guide', async () => ({
        contents: [{
    ...
    server.resource('kos-cpu-menu-format', 'kos://cpu-menu-format', async () => ({
        contents: [{
    ...
    server.resource('kos-transport-options', 'kos://transport-options', async () => ({
        contents: [{
    ...
    server.resource('kos-status', 'kos://status', async () => {
        const conn = getConnection();
    ...
    server.resource('kos-terminal-recent', 'kos://terminal/recent', async () => {
        const status = globalKosMonitor.getStatus();
        return {
            contents: [{
