 ...
 */
export async function timeWarpKick(conn, options = {}) {
    const { duration = 1000, warpRate = 1, postKickDelay = 500, } = options;
 ...
 */
export async function installTimeWarpKickTrigger(conn, condition, bufferSeconds = 10) {
    const triggerScript = `
    WHEN ${condition} THEN {
 ...
 * @param conn kOS connection
 */
export async function immediateTimeWarpKick(conn) {
 ...
 */
export async function delayedTimeWarpKick(conn, delaySeconds) {
    const script = `
