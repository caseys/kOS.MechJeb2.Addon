...
# Deploy kOS test scripts (kOS requires lowercase filenames)
SCRIPTS_DIR="/Volumes/Flatty/SteamLibrary/steamapps/common/Kerbal Space Program/Ships/Script"
if [ -d "$SCRIPTS_DIR" ]; then
    echo ""
    echo "Deploying kOS test scripts..."
    for ks_file in Tests/*.ks; do
        if [ -f "$ks_file" ]; then
            # Get filename and convert to lowercase
            filename=$(basename "$ks_file")
            lowercase_name=$(echo "$filename" | tr '[:upper:]' '[:lower:]')
            cp "$ks_file" "$SCRIPTS_DIR/$lowercase_name"
        fi
    done
    echo "✅ Test scripts deployed to kOS archive (as lowercase)"
    ls "$SCRIPTS_DIR"/*.ks 2>/dev/null | head -10
fi
